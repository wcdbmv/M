...
Container<Container<double>> build_coeff_matrix(const Container<Container<double>>& matrix) {
	...
	Container<Container<double>> result(n, Container<double>(n));
...
template <template <typename> typename Container>
Container<double> build_augmentation_matrix(typename Container<double>::size_type n) {
	Container<double> result(n);
	result[n - 1] = 1;
	return result;
}
...
Container<double> get_system_times(const Container<Container<double>>& matrix) {
	...
	const auto n = matrix.size();
	Container<double> result = gauss(build_coeff_matrix(matrix), build_augmentation_matrix<Container>(n));
